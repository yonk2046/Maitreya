...
def main() -> int:
    ...
    for d in dates:
        ...
        # D-7 HARD CONDITION: the replay contract must recognize the obs_landing
        # flag. An I-only backfill snapshot (obs_landing=false) was generated by
        # ingest's backfill mode (only I fields written, all O engines skipped).
        # Re-running ingest with the default obs_landing=True would recompute the
        # O layer (obs_* + sync_streak + market grain) that this snapshot never
        # had, guaranteeing a full-replay MISMATCH. Reading the on-disk flag and
        # threading it through makes the verifier reproduce the same mode, so
        # backfill snapshots replay clean instead of failing forever.
