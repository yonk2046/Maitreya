...
| 影響欄位 | `stocks[].weakening`、`velocity_3d`、`acceleration`、`fii_consecutive_buy_days`、`obs_sm_*`、`obs_golden_*`、`obs_chip_grade`、`obs_market_temperature`(讀當日 obs_sm)、`margin_balance`/`margin_change`(A5②,之前恆 None)、`price_down_margin_up_days_10d`/`_down_`(W4 輸入);sidecar `reports/strategy_tags/<date>.json`(讀該快照記錄的旗標) |
| 行為 | ① 缺席不透明:掉出主力榜的日子以主力買超 0 計,中斷連買/速度(G3)。但**缺席不觸發當日定案的「連買崩塌→FAILED」**(1 日缺席多為輪動,交給 W3/EXITED);外資連買改嚴格計算,缺席日視為中斷(缺席日外資實為未知,**刻意保守**);④ 個股融資落地(A5②)→ 資料完整度分級變動、W4 散戶接盤首次可觸發;② 快照轉弱用「前序 + 當天」判斷(G1,W3 不再亮在回榜日);③ 狀態機 CONFIRMED 廣度閘讀全市場 `market_pulse`,缺值不放行(G9) |
