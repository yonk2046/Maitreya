...
**查證結論(2026-07-14，回測 `reports/_daily_logs/*.log` 71 筆 + `reports/_raw_archive/*/legacy_today_json/today.json` 27 天實際資料)**：

- **原始描述「天天 0/10」不準確**：71 筆歷史 emit 記錄中，`外資榜重疊 0/10、主力榜重疊 0/5`
  精確符合的只有 19 筆(~27%)，其餘天數重疊數會隨真實資料波動(常見 4-10/10)。判斷「恆定失敗」
  的印象很可能來自寫此條目當下(2026-07-11)最近兩天剛好是 7/09(部分抓取，T86 僅 7,922 檔)與
  7/10(颱風假，T86 全空)——巧合連續兩天 0，被誤讀為常態。
- **但查到兩個真 bug，已修**（`tools/fetch_daily.py`，抽成純函數 `compute_cross_verify_overlap()`，
  測試見 `tests/test_cross_verify.py`）：
  1. **ETF 口徑不一致**（真因、影響最大）：Fubon ZGK_D/ZGK_F 抓取時已用 `is_etf()` 濾掉 ETF
     (`tools/fetch_fubon.py`)，但 T86 全市場榜單比對前未濾——0050/00631L/00918 這類 ETF
     常年高居三大法人買賣超「金額」排行前段，會把個股擠出 T86 的 top10/top5，系統性壓低
     重疊數。回測 27 天：**19 天的重疊數因此被低估**，多天濾除 ETF 後應為 10/10 卻算出個位數
     (例：7/07 原算 8/10 → 濾除後 10/10；7/13 原算 9/10 → 濾除後 10/10)。修法：比對前對 T86
     側同樣濾掉 ETF，兩邊口徑一致再比。
  2. **Tri-state 缺失（誠實性問題）**：T86 當天完全無資料(抓取失敗或非交易日)時，舊版靜默輸出
     `foreignOverlap=0, mainforceOverlap=0`，與「比對過、真的沒有重疊」在 log 上無法區分——
     這正是判準要求的「檢查失效」與「檢查失敗」是不同訊息。修法：新增 `cross_verify["status"]`
     (`ok` / `disabled`)，`disabled` 時 emit 訊息改為「多源驗證停用：T86 無資料」而非假分數。
- **未動、留待判斷（複雜/需架構決定，本次不動 code）**：「個股一致」(`perStock[].foreignMatch`)
  用 `|Δ| < 1%` 判定 Fubon 外資買超張數 vs T86 外陸資買賣超張數是否一致——即使 T86 有效資料的
  天數，這個嚴格 1% 門檻仍常年偏低(27 天中 15 天 matched=0)。根因疑似**兩源對「外資」的定義
  本身不同**(T86 明確排除外資自營商，Fubon ZGK_D 抓取頁面的口徑未證實是否同口徑；且兩者抓取
  時點/週期可能不同步)，抽樣看到的 delta% 常在 35%-140% 量級，不是雜訊等級的不一致，較像是
  結構性定義落差。這需要先確認 Fubon 頁面的精確欄位定義（沒有官方文件，只能反查數值規律）才能
  決定合理門檻或改用方向一致性(同買/同賣)取代嚴格數值比對——留給有時間深入這條的後續模型或
  Yonki 決定，不在本次「一行修法」範圍內。
