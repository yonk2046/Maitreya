...
> ⚠️ **2026-09-15:本節回測數字的撮合價有系統性錯誤(快照 `open` 落後一天,91% 成交價非真實開盤),單筆不可信、策略排名會變。見 §七。**
...
> ⚠️ **2026-09-15:凍結後的前推紀錄有缺口(7 個交易日無快照、9/4 混日),且評估用的回測引擎有 §七 所列錯誤。見 §七。**

### 為什麼是凍結而非補歷史
往回補資料**不可行**,三個獨立理由:

1. **原始觀測不存在**。`reports/_raw_archive/` 最早 2026-05-08,與首份快照同日 ——
   採集從那天開始,之前沒有任何可 replay 的原始觀測。
2. **分點歷史不可得**。Sinotrade 只提供近期分點;籌碼錨定家族(swing/v2/v3)的進場
   建立在主力成本錨與分點買賣超上 → **沒有歷史分點,這三支根本無法往回回測**
   (輸入不存在,不是精度問題)。
3. **補歷史必踩 look-ahead**。已證實實例:TDCC `load_for_date` 取「最近一個 ≤ 目標日的
   週五」,重建時會拿到當時實際不存在的當日檔(2026-07-24 查證)。往回補會系統性把
   事後資訊注射進每個決策點 —— 錯不在樣本獨立性,在**時點污染**,正是本架構(C10 as-was)
   花力氣在防的東西。

### 樣本外的三級與本案定位
| 級別 | 定義 | 本案 |
|---|---|---|
| 偽樣本外 | 設計時已可見,事後切線 | §五 D2 的 07-01~07-27 屬此級 |
| 真保留樣本 | 設計前鎖起、完成前不看 | 未曾建立 |
| **前推/實盤** | 凍結時資料尚不存在 | **自本宣告起適用** |

判準是「這段資料有沒有影響過設計決定」,不是日期。現存五支策略本身就是用這份語料
篩選過的結果(被淘汰的版本沒出現在任何表格裡,但已消耗掉這份資料的獨立性)——
故 §五 的切分結論只能當方向參考,不能當統計驗證。

### 凍結內容(HEAD `eb7fbaf`)
策略設定 hash(`dataclasses.asdict` → `canonical_sha256`):
...
回測參數(`BACKTEST_*`,研究層,不入 config_hash,故需在此明列):
`COOLDOWN_DAYS=2`、`COST_BREAK=0.92`、`COST_CAP=1.15`、`COST_FULL_TIER=1.05`、
`ENTRY_STOP=0.93`、`ADD_MIN_PRICE_MULT=0.98`、`FEE_RATE=0.000855`、`FEE_MIN=20.0`、
`TAX_RATE=0.003`、`POSITION_SIZE=100000.0`、`INITIAL_CAPITAL=1000000.0`。
`engine_params.as_config_dict()` hash:`sha256:abf994d242bebd680fe6fc0…`

### 凍結規則
- **2026-07-28 之後的每個交易日 = 真前推樣本外**(凍結時該資料尚不存在,任何人都不可能
  用它調過參數)。pipeline 每日照算 → 等於免費跑 paper trading。
- **動到上表任一值 = 前推紀錄作廢、計時歸零**,必須在本節補記新的凍結日與 hash。
  守門員:`tests/test_strategy_freeze.py`(改設定即紅,強迫承認這件事)。
- **評估閘門**:累積約 60 個前推 session(~3 個月,約 2026-10 底)再回頭評。
  屆時拿到的不是回測,是實際前推紀錄 —— 說服力高於任何事後切分。
- 在此之前**不得依 §五 數字上線**:n=4~11、標的集中金融+航運兩族、且樣本外切分
  漏掉 v3 唯一虧損單(2885 元大金 −7.25%,見 §五 D2 保留)。

---

## 七、回測可信度檢查與凍結紀錄缺口(2026-09-15)

### 7.1 撮合價錯誤:快照 `open` 落後一天
- 來源:`data/adapters/legacy.py:450-455` 取 `today.json["openPrices"]`(TWSE STOCK_DAY_ALL OpenAPI)。
- 撮合:`core/paper_trading.py:103-108 _fill_price` = 下一份快照的 `open`,缺值退回 `current_price`。
- 範圍查證(5 檔高頻個股 × 81 份快照,對 TWSE STOCK_DAY 官方日行情):
  - 18–19 點例行建置:**95.3%** 的 `open` 為前一交易日開盤;隔日上午建置 85.7% 正確。
  - `current_price` 對當日收盤命中 92.9%(同日正常建置 100%)→ 問題集中在 `open`。
- 148 筆回測交易:**91.2% 進場價非真實進場日開盤**(45.3% = 前一日開盤,即訊號日當天早上的價格;43.2% = open 缺值退回收盤)。
- 以 TWSE 真實開盤重算(毛報酬、不含成本、排除 end_of_data 6 筆):

| 策略 | 可重算 | 原 | 真實開盤 | 差 | 勝率 |
|---|---|---|---|---|---|
| chip_anchored_swing | 27 | −0.96% | +0.06% | +1.02pp | 51.9%→63.0% |
| chip_anchored_v2 | 39 | −1.34% | −0.53% | +0.81pp | 46.2%→56.4% |
| chip_anchored_v3 | 10 | +3.46% | +3.06% | −0.40pp | 90.0%→90.0% |
| momentum_continuation | 30 | −0.11% | −0.40% | −0.29pp | 53.3%→43.3% |
| momentum_v2 | 36 | −0.45% | −0.48% | −0.02pp | 50.0%→41.7% |

- 解讀:**方向不一,不是一律高估**。籌碼錨定 swing/v2 被低估,動能家族被高估;v3 大致站得住但 n=10。單筆最大誤差 ~15pp(2344 華邦電 8/5→8/21:記錄 +20.00%,真實 +5.39%;進場用了 8/4 開盤、出場用了 8/20 開盤)。
- 抽查:2303 聯電 9/3 快照 `open=131.5` = TWSE 9/2 開盤(9/3 真實 127.50);2344 案例已以 TWSE 快取逐筆核對。

### 7.2 回測引擎成本立方成長
`run_backtest` 對 chip_anchored 每日 `golden.run(snaps[:i+1])` 不快取。CI 生產 log:5s(6/29)→27s(7/28)→82s(8/21)→85s(9/4)。測試端已隔離(`4555161`,11 顆標 slow);**pipeline 端推估 2026-12 ~ 2027-01 撐破 job 上限**(FORWARD-RISK-REGISTER R13)。

### 7.3 回補批次快照品質(WORM,不可改,定基準時須標註)
- 5/08–5/25(5/27 雲端批次)、5/26–7/09(7/14 Mac 批次):`open` 幾乎全 null;部分日(如 6/12)`current_price` 偏移一日。
- 7/20、7/30、8/10:正常建置但 `open` 全 null(STOCK_DAY_ALL 當日抓取失敗)。

### 7.4 凍結前推紀錄的缺口
- 凍結日 2026-07-28 之後:**9/2、9/7、9/8、9/9、9/10、9/11、9/14 無正式快照**(見 `docs/handoffs/MAITREYA_HANDOFF_20260915.md` §1)。
- **9/4 快照混日**:富邦主力榜系欄位為 9/7,T86/MI20/融資為 9/4。選股範圍(26 檔)本身即來自 9/7 主力榜;分點標 `fetched_date=2026-09-04`,但 9/8 01:42 抓取、Sinotrade 只給最新一天 → 實為 9/7(未逐筆驗)。由上述欄位衍生的 O 態(`obs_*`、tier、gates、temporal)同受污染。
  **D1 裁定(2026-09-22,Yonki):9/4 只註記、不重建。** 快照(WORM)不動;9/4 列為**已知不可用日**:前推評估不計入,T1 回測不以 9/4 作進場/出場撮合日。理由:真 9/4 主力榜已永久遺失(連 26 檔選股都是 9/7 榜),重建只能把錯值換成空值;且 9/15 起每份快照的 `lookback_snapshots` 都含 9/4 → supersede 會連帶 9/15–9/24 共 8 份一起重建、改寫前推紀錄。
- 9/14 原始資料已救援至 `/Users/yoncky/SCD engine/_rescue/2026-09-14/`(非正式快照)。
- 策略設定本身未變動(`tests/test_strategy_freeze.py` 仍綠),凍結宣告有效;但 10 月底評估時可用 session 數減少,且評估須在 7.1/7.2 修正後進行。

### 7.5 下一步(需在 2026-11 底前完成)
1. `_fill_price` 改用按日期查詢的真實日行情開盤(不讀快照 `open`);
2. `run_backtest` 增量化,消除每日重算整段歷史;
3. 兩者**一起做、一次重定基準**,配黃金基準測試防止無聲改變結果;新基準記入本檔 §八;
4. 回測 JSON `limitations` 更新(現文「snapshots carry no open」已過時)。
