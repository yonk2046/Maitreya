...
2. **replay 的新參數陷阱**(`tools/verify_all_replay.py::_params_as_recorded`):舊快照沒記錄的參數,replay 時用**現值**。
   若新增「修正開關=開」,所有舊 1.9.0 快照 replay 時會套上新邏輯 → 全面不符 → pipeline 拒絕發布(7/16–17 斷更同型)。
   → **不 bump 的修正一律「依快照日期生效」**:參數 `ENGINE_CORRECTION_EFFECTIVE = "<生效日>"`,快照日期 < 生效日走舊邏輯。舊快照 replay 不受影響,新快照照常記錄參數。
...
| A3 | 狀態機廣度閘改讀 `obs_market_breadth`(全市場母體) | G9 | **判斷改變,需 Yonki 另行點頭**;預設不做 |
...
生效日建議:A1/A2/A4 一起在**某個週一**生效(例 2026-10-05),前一個週末完成部署與全量驗證(週末無排程壓力)。
